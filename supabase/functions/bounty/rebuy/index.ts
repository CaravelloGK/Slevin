// Supabase Edge Function: bounty/rebuy
// POST /functions/v1/bounty/rebuy
// Body: { tournament_id, player_id }

import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'
import { processRebuy, validateRebuyRequest } from './logic.ts'
...
Deno.serve(async (req: Request) => {
  ...
  const validated = validateRebuyRequest(body)
  if (!validated) {
  ...
  const result = await processRebuy(validated, {
    supabase,
