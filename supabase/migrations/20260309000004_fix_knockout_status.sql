-- Fix: remove 'rebought' from process_knockout status checks.
-- 'rebought' was never a valid value in the player_status enum in the DB.
-- After any rebuy the status is set back to 'active', so checking only 'active' is correct.
...
begin
  ...
  -- Count all active players — this determines the victim's finishing position
